...
test('state endpoint fails closed without verified access context',async()=>{const {onRequestGet}=await state(); const res=await onRequestGet(ctx()); assert.equal(res.status,403);});
